...
riemann sums and went straight to integrals. Perhaps there was a small exercise but I recall applying the idea more in my 
...
theorem due to the function being uniformly convergent is that given a sequence of continuous functions $f_n(x)$, one could do the following: $\lim \limits_{n\to\infty} \int_a^b f_n(x)dx = \int_a^b f(x)dx$.
